...
class db
{
	...
	// FAQ category counts
	function faqCategoryNos($cat_id, $sign, $amount = 1) {
		...
		if($cat_id > 0) {
			...
			do {
				...
				$record['noFAQs'] = " noFAQs ".$sign.$amount;
				$where = "faqCat_id = ".$cat_id; 
				$this->update($glob['dbprefix']."CubeCart_faq_category", $record, $where, "");
			
				$query = "SELECT faqCat_father_id FROM ".$glob['dbprefix']."CubeCart_faq_category WHERE faqCat_id = ".$cat_id;
				$cfi = $this->select($query);
				$cat_id = $cfi['0']['faqCat_father_id'];
	...
	// FAQ Management System by MarksCarts, http://cc3.biz - END
