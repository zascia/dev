...
////////
function getCatDir($catName, $cat_father_id, $catId, $link=FALSE, $last_link=FALSE){
...
function listModules($path){
	...
	if ($handle = opendir($path)) {
	   ...
	   while (false !== ($folder = readdir($handle))) {
			...
			if(!eregi($folder, ".|error_log|index.php")){
...
// FAQ Management System by MarksCarts, http://cc3.biz - START
#######################
###    FAQ-SPECIFIC FUNCTIONS
#######################
//////////////////////////////////
// Get FAQ Category Directory 
////////
function getFAQDir($faqCatName, $faqCat_father_id, $faqCatId, $link=FALSE){
	
	global $sessId,$db,$config,$glob,$lang_folder;
	
	// get category array for cat dir
	$faqQuery = "SELECT faqCat_id, faqCat_name, faqCat_father_id FROM ".$glob['dbprefix']."CubeCart_faq_category ORDER BY faqCat_id DESC";
	$faqCatArray = $db->select($faqQuery);
	
	// get category array in foreign innit
	$faqResultsForeign = $db->select("SELECT faqCat_master_id as faqCat_id, faqCat_name FROM ".$glob['dbprefix']."CubeCart_faq_cats_lang WHERE faqCat_lang = '".$lang_folder."'");
	
	$faqDirArray[0] = $config['dirSymbol'].$faqCatName;
	
	for ($i=0; $i<=count($faqCatArray); $i++) {
		
		if(is_array($faqResultsForeign)){

			for ($k=0; $k<count($faqResultsForeign); $k++){

				if($faqResultsForeign[$k]['faqCat_id'] == $faqCatArray[$i]['faqCat_id']){
				
					$faqCatArray[$i]['faqCat_name'] = validHTML($faqResultsForeign[$k]['faqCat_name']);
		...
		if(isset($faqCatArray[$i]['faqCat_id']) && $faqCatArray[$i]['faqCat_id']==$faqCat_father_id) {
			
			if($link == TRUE){
			
				$faqDirArray[$i+1] = $config['dirSymbol']."<a href='".$GLOBALS['rootRel']."index.php?ccUser=".$sessId."&amp;faqCatId=".$faqCatArray[$i]['faqCat_id']."&amp;act=viewFAQs' class='txtLocation'>".$faqCatArray[$i]['faqCat_name']."</a>";
			
			} else {
			
			$faqDirArray[$i+1]= $config['dirSymbol'].$faqCatArray[$i]['faqCat_name'];
			
			}
			
			$faqCat_father_id = $faqCatArray[$i]['faqCat_father_id'];
	...
	foreach($faqDirArray as $key => $value){
	 	$dir.= $value;
	}
...
//////////////////////////////////
// get alternate language of faq
////////
function faqAltLang($faqId){
	
	global $db, $lang_folder, $glob, $config;
	
	if($lang_folder !== $config['defaultLang']){
	
		$foreignVal = $db->select("SELECT question, answer FROM ".$glob['dbprefix']."CubeCart_faqs_lang WHERE faq_master_id = ".$db->mySQLSafe($faqId)." AND faq_lang=".$db->mySQLSafe($lang_folder));
