...
var loadFaceBookApi = function () {
    try {
        if (jQuery('.fb-comments').length > 0) {
            //console.log('[facebook-comments-done]');
            (function (d, s, id) {
                var js, fjs = d.getElementsByTagName(s)[0];
                if (d.getElementById(id)) return;
                js = d.createElement(s); js.id = id;
                js.src = "https://connect.facebook.net/pt_PT/sdk.js#xfbml=1&version=v2.12&appId=352742014914127";
                fjs.parentNode.insertBefore(js, fjs);
            }(document, 'script', 'facebook-jssdk'));
        }
    } catch (e) {
        console.log('[error: facebookBootUp]');
    }
}

var faceBookCommentsRefresh = function () {
    try {
        var fbCommentDiv = jQuery('.fb-comments')[0];
        if (fbCommentDiv) { FB.XFBML.parse(fbCommentDiv.parentNode); }
    } catch (e) {
        console.log('[error: faceBookCommentsRefresh]');
    }
}
...
function SetMeteoChoice(city) {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            jQuery(".t-mb-forecast-today").remove();
            jQuery(".t-mb-forecast-future").remove();
            jQuery(data).appendTo(".t-mb-forecast-local");
            SetMeteoSummary(city);
            SaveCookie("MeteoCityCookie", city);
        },
