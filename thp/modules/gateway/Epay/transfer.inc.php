...
function fixedVars() {
	...
	$amount = sprintf("%.2f",$basket['grandTotal'])*100;
	...
	$hiddenVars = "<input type='hidden' name='merchantnumber' value='".$module['merchant']."' />
				...
				<input type='hidden' name='accepturl' value='".$GLOBALS['storeURL']."/accepted.php' />
				<input type='hidden' name='cancelurl' value='".$GLOBALS['storeURL']."/confirmed.php?act=conf&amp;oid=".base64_encode($cart_order_id)."' />";


				if($module['method']=="callback") $hiddenVars .="<input type='hidden' name='callbackurl' value='".$GLOBALS['storeURL']."/modules/gateway/Epay/call_back.php' />\r\n<input type='hidden' name='cb_dibs' value='yes' />
				";
