...
*/
	...
	$module = fetchDbConfig("Forudbetaling_via_Bank");
	...
	## FAKTURA >>
	if (!empty($result[0]['invoice'])) {
	   $print_order_form->assign("VAL_FAKTURA",$result[0]['invoice']);
	   $print_order_form->assign("TXT_FAKTURA",$lang['convict_invoice']['invoice_no']);
	   $print_order_form->parse("order_form.faktura");
	}
	## <<
...
//.: adg_coupon_mod http://www.alexgoldberg.com/cubemods :.
	...
	if ($adg_gd_module['status']==1 && $adg_gd_module['show_group']==1 && $ccUserData[0]['group_discount'] >0) {
		$print_order_form->assign("LANG_GROUP",$lang['mods']['adg_group_mod']['front_discount_group']);
		$print_order_form->assign("TXT_GROUP_NAME",$ccUserData[0]['group_name']);
		$print_order_form->assign("LANG_YOUSAVE",$lang['mods']['adg_group_mod']['front_you_save']);
		$print_order_form->assign("TXT_DISCOUNT",$ccUserData[0]['group_discount']);
		$print_order_form->parse("order_form.show_discount_group");
	}
	//.: Group Discounts Mod by Goober http://www.cubecartmodder.com/
