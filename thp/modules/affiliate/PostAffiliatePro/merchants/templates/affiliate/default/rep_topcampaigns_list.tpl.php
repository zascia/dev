...
<tr>
  <td height=150 valign=bottom width="100%" align="center">
...
<tr>
  ...
  <table class=listing border=0 cellspacing=0 cellpadding=1 style="border-top: 0px; border-bottom: 0px;" width="80%" align=center>
    <tr>
        ...
        <td class=listheaderLineTop><?php echo L_G_CAMPAIGN?></td>
  ...
  <?php foreach($this->a_impstop_data as $k => $data) { 
        if($data['campaignid'] == '0' && $data['all'] == 0) continue; // skip empty others
  ?>
        <tr class="listrow<?php echo ($k++)%2?>">
            ...
            <td class=listresultnocenter align=left>
                    <font color="#0000ff">&nbsp;<?php echo $data['campaignid'].': '.$data['name']?></font>
                    <?php  if($data['campaignid'] != '' && $data['campaignid'] != '0') { 
                            showQuickDetails("index_popup2.php?md=Affiliate_Merchants_Views_CampaignManager&action=campdetails&cid=".$data['campaignid'], 300);
                        }
                    ?>
            </td>
  ...
  <?php } 
     if(count($this->a_impstop_data) <= 1 && $this->a_impstop_data[0]['campaignid'] == 0) {
?>
        <tr class="listrow2">
...
<tr>
  ...
  <table class=listing border=0 cellspacing=0 cellpadding=1 style="border-top: 0px; border-bottom: 0px;" width="80%" align=center>
    <tr>
        <td class=listheaderLineTop>&nbsp;</td>
        <td class=listheaderLineTop><?php echo L_G_CAMPAIGN?></td>
        <td class=listheaderLineTop width="10%"><?php echo L_G_CPMCOMMISSIONTRANSACTIONS?></td>
    </tr>  
  <?php foreach($this->a_cmptop_data as $k => $data) { 
        if($data['campaignid'] == '0' && $data['count'] == 0) continue; // skip empty others
  ?>
  ...
  <?php } 
     if(count($this->a_cmptop_data) <= 1 && $this->a_cmptop_data[0]['campaignid'] == 0) {
?>
        <tr class="listrow2">
            <td class=listresultnocenter colspan=3 align=center>
            <?php echo L_G_NODATA?>
...
<tr>
  ...
  <?php } 
     if(count($this->a_clickstop_data) <= 1 && $this->a_clickstop_data[0]['campaignid'] == 0) {
?>
        <tr class="listrow2">
            <td class=listresultnocenter colspan=3 align=center>
            <?php echo L_G_NODATA?>
...
<tr>
  ...
  <?php } 
     if(count($this->a_salestop_data) <= 1 && $this->a_salestop_data[0]['campaignid'] == 0) {
?>
        <tr class="listrow2">
            <td class=listresultnocenter colspan=3 align=center>
            <?php echo L_G_NODATA?>
...
<tr>
  ...
  <?php } 
     if(count($this->a_leadstop_data) <= 1 && $this->a_leadstop_data[0]['campaignid'] == 0) {
?>
        <tr class="listrow2">
            <td class=listresultnocenter colspan=3 align=center>
            <?php echo L_G_NODATA?>
...
<tr>
  ...
  <?php foreach($this->a_revenuetop_data as $k => $data) { 
        if($data['campaignid'] == '0' && $data['sum'] == 0) continue; // skip empty others
  ?>
  ...
  <?php } 
     if(count($this->a_revenuetop_data) <= 1 && $this->a_revenuetop_data[0]['campaignid'] == 0) {
?>
        <tr class="listrow2">
            <td class=listresultnocenter colspan=3 align=center>
            <?php echo L_G_NODATA?>
