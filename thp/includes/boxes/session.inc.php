...
$result = $db->select("SELECT * FROM ".$glob['dbprefix']."CubeCart_customer WHERE customer_id = ".$ccUserData[0]['customer_id']." and mobile='8888888888'");
	...
	if($result == TRUE) {
$box_content->assign("LANG_WELCOME_BACK",$lang['front']['boxes']['back_welcome']);
		$box_content->assign("TXT_USERNAME",$ccUserData[0]['firstName']);
		$box_content->assign("VAL_SELF",base64_encode(currentPage()));
		$box_content->assign("LANG_REGISTER",$lang['front']['boxes']['register']);
		$box_content->parse("session.session_express");
		}
