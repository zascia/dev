...
if (eregi(".inc.php",$HTTP_SERVER_VARS['PHP_SELF']) || eregi(".inc.php",$_SERVER['PHP_SELF'])) {
	...
	if($order == TRUE){
	    ...
	    ## FAKTURA >>
	  	if (!empty($order[0]['invoice'])) {
            $view_order->assign("YOUR_FAKTURA",$lang['convict_invoice']['invoice_no'].": ".$order[0]['invoice']);
            $view_order->parse("view_order.faktura");
        }
	    ## <<
	...
	// start mod: Shipping Tracking Number, by Estelle
		$stn_template_parse = "view_order.session_true.order_true.tracking_number";
		include("includes/estelles_mod_store/shipping_tracking_number.vieworder1.inc.php");
	// end mod: Shipping Tracking Number, by Estelle
