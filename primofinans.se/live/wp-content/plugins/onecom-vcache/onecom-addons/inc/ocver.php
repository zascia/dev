	...
	class OCVer {
		...
		*/
		public function checkItemStatus() {
			...
			// inject validator
			$features = oc_validate_domain();
			if (isset($features['data'])){
				$features = $features['data'];
			}		
			if (! in_array('PERFORMANCE_CACHE', $features)){
				$this->updateTransient( $name, 'false' );
				$this->access = $this->can_access( 'false' );
				return;
			}else{
				$this->updateTransient( $name, 'true' );
				$this->access = $this->can_access( 'false' );
				return;
			}

			// @todo: the block below is now unreachable, need to remove this after discussion
