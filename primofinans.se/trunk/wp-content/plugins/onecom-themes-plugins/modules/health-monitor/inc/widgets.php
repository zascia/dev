...
function osch_widget_cb()
{
    ...
    wp_add_dashboard_widget(
        WID,
        __('One.com Health Monitor', OC_PLUGIN_DOMAIN),
        'ocsh_widget_cb'
    );
...
function ocsh_widget_cb()
{
    ...
    if ( ! $site_scan_result ){
        echo '<p><a class="button button-primary" href="'.$url.'">'.__('Scan now', OC_PLUGIN_DOMAIN).'</a></p>';
        return;
    }
    $color = '#4ab865';
    ...
    echo '<p>'.__('Health Monitor score is', OC_PLUGIN_DOMAIN).' <strong style="color:'.$color.'">'.round($site_scan_result['score']).'%</strong> ('.__('Last checked on', OC_PLUGIN_DOMAIN).' '.date_i18n($date_format.' '.$time_format, $site_scan_result['time']).')</p><p><a class="button button" href="'.$url.'">'.__('Scan again', OC_PLUGIN_DOMAIN).'</a></p>';
}
